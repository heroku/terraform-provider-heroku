...
func resourceHerokuDrainImport(d *schema.ResourceData, meta interface{}) ([]*schema.ResourceData, error) {
	...
	dr, err := config.Api.LogDrainInfo(context.Background(), app, id)
	if err != nil {
		return nil, err
	}
...
func resourceHerokuDrainCreate(d *schema.ResourceData, meta interface{}) error {
	...
	err := resource.Retry(2*time.Minute, func() *resource.RetryError {
		d, err := config.Api.LogDrainCreate(context.TODO(), app, heroku.LogDrainCreateOpts{URL: url})
		if err != nil {
...
func resourceHerokuDrainDelete(d *schema.ResourceData, meta interface{}) error {
	...
	// Destroy the drain
	_, err := config.Api.LogDrainDelete(context.TODO(), d.Get("app").(string), d.Id())
	if err != nil {
...
func resourceHerokuDrainRead(d *schema.ResourceData, meta interface{}) error {
	...
	dr, err := config.Api.LogDrainInfo(context.TODO(), d.Get("app").(string), d.Id())
	if err != nil {
